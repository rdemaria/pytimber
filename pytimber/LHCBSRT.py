...
try:
    ...
    print(("No module found: pandas, numpy, matplotlib and scipy modules "
...
def _timber_to_emit(bsrt_array):
    ...
    unit_rescale = 1
    bsrt_prior['emith'] = (bsrt_prior['sigh']**2 - (bsrt_prior['lsfh'] /
        unit_rescale)**2) / bsrt_prior['beth']
    bsrt_prior['emitv'] = (bsrt_prior['sigv']**2 - (bsrt_prior['lsfv'] /
        unit_rescale)**2) / bsrt_prior['betv']
    unit_rescale = 1000
    bsrt_post['emith'] = (bsrt_post['sigh']**2 - (bsrt_post['lsfh'] /
        unit_rescale)**2) / bsrt_post['beth']
    bsrt_post['emitv'] = (bsrt_post['sigv']**2 - (bsrt_post['lsfv'] /
        unit_rescale)**2) / bsrt_post['betv']
    ...
    bsrt_array['emith'] = bsrt_array[['emith',
                                      'energy']].apply(func, axis=1)
    bsrt_array['emitv'] = bsrt_array[['emitv',
                                      'energy']].apply(func, axis=1)
    return bsrt_array
...
class BSRT(object):
    ...
    def __init__(self, db=None, beam=None, emit=None, emit_fit=None,
    ...
    def fromdb(cls, t1, t2, beam='B1', db=None,
        ...
        return cls(db=db, emit=emit, emit_fit=None,
    ...
    def update_beta_lsf_energy(self, t1, t2, beth=None, betv=None,
        ...
        mask = np.logical_and(bsrt_array['time'] >= t1,
                              bsrt_array['time'] <= t2)
...
< t_end.""")
        ...
        def fit_slot(group):
            ...
            if (self.emit_fit is not None and force is False and
               slot in self.emit_fit.index.get_level_values(0)):
            ...
            for plane in ['h', 'v']:
                ...
                if epst2_fit < 0:
                    err_str = ("Invalid value of BSRT emittance "
                               "(eps < 0) for time "
                               "t2={}").format(parsedate(time[-1]))
                    raise ValueError(err_str)
                if epst1_fit < 0:
                    err_str = ("Invalid value of BSRT emittance "
                               "(eps < 0) for time "
                               "t1={}").format(parsedate(time[0]))
                    raise ValueError(err_str)
                ...
                popt, pcov = curve_fit(exp_fit, time,
                                       group[emit_key],
                                       p0=[a_init, tau_init])
    ...
    def _set_times(self, t1, t2, verbose):
        ...
        if t2 > self.t_end:
            err_str = ('End time t2 = {} > {} '
                       'lies outside of data range!').format(t1, self.t_end)
            raise ValueError(err_str)
        if t2 < t1:
            err_str = ('End time smaller than start time, t2 = '
                       '{} > {} = t1').format(t2, t1)
            raise ValueError(err_str)
        return t1, t2

    def plot(self, plane='h', t1=None, t2=None, slots=None, avg=10, fit=True,
             color=None, label=None, verbose=False, err_bar=False):
        ...
        for slot in slots:
            ...
            if color is None:
                ...
                c = color
            mask = np.logical_and(self.emit.loc[slot].index >= t1,
                                  self.emit.loc[slot].index <= t2)
            eps = self.emit.loc[slot][mask]
            # raw data
            if avg is None:
                pl.plot(eps.index, eps['emit{}'.format(plane)],
                        '.', color=c, label=label)
            # averaged data
            else:
                if len(eps) < avg:
                    if verbose:
                        warn_str = ('slot {} number of measurements: {} < '
