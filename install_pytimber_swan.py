...
install_loc = os.path.join(
    os.environ["HOME"], "python", "environments", "pytimber3"
)
...
if not os.path.exists(install_loc):
    print(
        "Creating an environment in {}. This can take a while.".format(
            install_loc
        )
    )
    ...
    env = {
        "PIP_INDEX_URL": "http://acc-py-repo.cern.ch:8081/repository/vr-py-releases/simple",
...
if not pytimber.__version__.startswith("3."):
    print(
        "Error installing newer version of PyTimber. "
